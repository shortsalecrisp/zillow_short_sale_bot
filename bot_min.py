...
def google_lookup(agent: str, state: str, broker: str) -> tuple[str, str]:
    if not GOOGLE_API_KEY or not GOOGLE_CSE_ID:
        return "", ""
    for q in (f'"{agent}" {state} phone email',
              f'"{agent}" \"{broker}\" phone email' if broker else ""):
        if not q:
            continue
        params = {"key": GOOGLE_API_KEY, "cx": GOOGLE_CSE_ID,
                  "q": q, "num": 10}
        for it in requests.get("https://www.googleapis.com/customsearch/v1",
                               params=params, timeout=10).json().get("items", []):
    ...
    if APIFY_TOKEN:
        try:
            ...
            if isinstance(resp, list) and resp:
                rec = resp[0]
                return (rec.get("mobilePhone") or rec.get("officePhone") or "",
                        rec.get("email") or "")
        except Exception:
            pass
    return "", ""
...
def process_rows(rows):
    ...
    for row in rows:
        ...
        detail_url = (row.get("detailUrl") or row.get("url") or
                      row.get("hdpData.homeInfo.detailUrl") or "")
        listing_text = (row.get("homeDescription") or row.get("description") or
                        row.get("hdpData.homeInfo.homeDescription") or
                        row.get("hdpData.homeInfo.description") or
                        zillow_description(detail_url))
        if not listing_text.strip():
            listing_text = " ".join(str(v) for v in row.values()
                                    if isinstance(v, str))

        prompt = ("Return YES if the following text contains the phrase "
                  "'short sale' (case-insensitive) and does NOT contain any "
                  "of: approved, negotiator, settlement fee, fee at closing. "
                  "Otherwise return NO.\n\n" + listing_text)
        ...
        if not phone and not email:
            logger.warning("%s – contact not found for %s", zpid, agent)
