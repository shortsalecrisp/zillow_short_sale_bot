...
_sms_api_key = os.getenv("AUTOREMOTE_KEY") or os.getenv("SMS_GATEWAY_API_KEY") or os.getenv("SMS_API_KEY", "")
...
SMS_TEMPLATE      = (
    "Hey {first}, this is Yoni Kutler with Crisp Short Sales. I saw {address} "
    "listed as a short sale. I can take the lender paperwork, calls, and "
    "follow-up off your plate. There's no service fee to you or the seller; "
    "my fee is buyer-paid at closing. Are you handling the lender side "
    ...
    *,
    ...
    for _ in range(max_attempts):
        ...
        try:
            ...
            if allow_fallback and len(raw_links) < limit:
    ...
    *,
    ...
    for r in rows:
        ...
        if _pilot_origin_requires_verifier_hold(r):
            ...
            outcomes[zpid] = "verifier_held"
            LOG.info(
                "PILOT_VERIFIER_HOLD zpid=%s address=%s source_agent_hint_present=%s "
                "sheet1_writes=0 contact_enrichment=false initial_sms_scheduled=false",
                zpid,
                r.get("street", ""),
