...
def _follow_up_pass():
    ...
    start_row_idx=max(2,last_row_idx-int(FU_HOURS)+1)
    ...
    for sheet_row,row in enumerate(recent_rows,start=start_row_idx):
        ...
        if row[COL_MANUAL_NOTE].strip() or row[COL_FU_TS].strip():
            continue
        try:
            ...
            continue
        if (now-ts).total_seconds()<FU_HOURS*3600:
            continue
        send_sms(
            phone=row[COL_PHONE],
            first=row[COL_FIRST],
        ...
        batch_updates.append({
            "range":f"Sheet1!X{sheet_row}",
            "values":[[datetime.now(tz=TZ).isoformat()]]
        })
    if batch_updates:
        sheets_service.spreadsheets().values().batchUpdate(
            spreadsheetId=GSHEET_ID,
