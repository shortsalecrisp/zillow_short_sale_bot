...
def fetch(url:str)->str|None:
    for u in (url,f"https://r.jina.ai/http://{url}"):
        try:
            ...
            if r.status_code!=200 and r.status_code not in (403,429,999):continue
            t=r.text
            if "unusual traffic" in t[:600].lower():continue
            return t
        except Exception:continue
    return None
...
def lookup(a:str,s:str)->tuple[str,str]:
    ...
    for q in build_q(a,s):
        ...
        for it in items:
            ...
            if tel:
                p=fmt_phone(tel)
                if valid_phone(p):cand_p[p]=(4,8)
            if mail and ok_email(mail):cand_e[mail]+=3
        for it in items:
            u=it.get("link","")
            t=fetch(u)
    ...
    if not phone and cand_p:phone=list(cand_p.keys())[0]
...
def extract_name(txt:str)->str|None:
    ...
    if m:
        nm=m.group(1).strip()
        if not TEAM_RE.search(nm):return nm
    return None
