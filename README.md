...
SMS sending uses the Tasker AutoRemote-compatible Android gateway. Provide the API key with the `AUTOREMOTE_KEY` or `SMS_GATEWAY_API_KEY` environment variable. Do not commit real SMS or email credentials to `config.json`; local config belongs in an ignored `config.json`, and Render credentials belong in the service environment. The provider can be set with `SMS_PROVIDER`, but currently only `android_gateway` is supported.
...
After a follow-up text is sent, the hourly scheduler waits at least two hours before marking column `K` with the
Mailshake code `N`. Rows are only released when `I=x`, both `J` and `K` are blank, and the voice-call lifecycle is
complete: a terminal first-call result, or a completed second attempt after a retryable first-call result. This keeps
Mailshake from claiming a row before its listing-local call window. A 72-hour failsafe releases a row if the voice
provider or scheduler never produces a terminal result. If a matching inbound SMS reply exists on the `Replies` tab,
the row is treated as replied instead and is not marked `N`.

You can tune this handoff with:

* `MAILSHAKE_AFTER_FOLLOWUP_HOURS` (default `2`)
* `MAILSHAKE_VOICE_MAX_WAIT_HOURS` (default `72`)
