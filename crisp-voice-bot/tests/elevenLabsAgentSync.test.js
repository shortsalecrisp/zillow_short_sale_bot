...
test("agent sync enables skip_turn for placeholder-only noise turns", () => {
  ...
  assert.match(script, /const FIRST_MESSAGE = '<break time="1\.0s" \/> Hey, is this {{firstName}}\?';/);
...
test("agent sync uses natural low-latency TTS settings", () => {
  ...
  assert.match(config, /voiceAbTestEnabled:\s*readBoolean\("ELEVENLABS_VOICE_AB_TEST_ENABLED", true\)/);
